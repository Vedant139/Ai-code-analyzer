...
const commScore = document.querySelector("#commScore");
const commBar = document.querySelector("#commBar");
const techScore = document.querySelector("#techScore");
const techBar = document.querySelector("#techBar");
const confScore = document.querySelector("#confScore");
const confBar = document.querySelector("#confBar");
const psScore = document.querySelector("#psScore");
const psBar = document.querySelector("#psBar");
...
function renderEvaluation(report) {
  ...
  if (commScore) commScore.textContent = report ? `${report.communication}/10` : "--";
  if (commBar) commBar.style.width = report ? `${Math.min(100, report.communication * 10)}%` : "0%";

  if (techScore) techScore.textContent = report ? `${report.technicalSkills}/10` : "--";
  if (techBar) techBar.style.width = report ? `${Math.min(100, report.technicalSkills * 10)}%` : "0%";

  if (confScore) confScore.textContent = report ? `${report.confidence}/10` : "--";
  if (confBar) confBar.style.width = report ? `${Math.min(100, report.confidence * 10)}%` : "0%";

  if (psScore) psScore.textContent = report ? `${report.problemSolving}/10` : "--";
  if (psBar) psBar.style.width = report ? `${Math.min(100, report.problemSolving * 10)}%` : "0%";
...
function startRecognitionInstance() {
  ...
  micIndicator?.classList.remove("is-hidden");
...
function stopListening() {
  ...
  state.interimText = "";
  micButton.textContent = "Start Microphone";
  micIndicator?.classList.add("is-hidden");
}
